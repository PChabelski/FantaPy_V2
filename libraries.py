...
def yahoo_data_collection(season, iter_date, year, schedule, control_file, query):
    ...
    league_id_split = control_file['Years'][str(year)]['league_id']
    game_id_split = control_file['Years'][str(year)]['game_id']
    count = 0
    df_rosterstats = pd.DataFrame()
    df_rosterstats_main = pd.DataFrame()
    str_error = None
        ...
        for date_item in gameRegisterDf['Date'].unique():
            ...
            if control_file["Online Parser Status"]['yahoo_parse']:
                try:
                    ...
                    yahoo_df.to_csv(f'{root_dir}ONLINE_PARSED_DATA/YH_ROSTERS/{year}/YH_stats_{date_item}.csv', index=False)
                except Exception as e:
            ...
            if control_file["Online Parser Status"]['nst_parse']:
                try:
                    ...
                    nst_df["Position_NST"].fillna('G', inplace=True)
...
def matchup_metadata(year, control_file):
    if control_file['Matchup Metadata']:
        ...
        for week_num in range(1, weeks['week'].max()):
...
def scheduleParser(year, control_file):
    if control_file['NHL Schedule Parser'] == True:
        ...
        for i in range(0, len(items)):
            # skip 0 to 3
            # 4 (date),5 (team A),6 (team B), skip 7
            # 8,9,10, skip 11
            # 12, 13, 14, skip 15
            if i < 4:
                continue
    ...
    if control_file['Stitcher Status']:
        ...
        for date in dates_of_interest:
            for file_name in glob.glob(f'ONLINE_PARSED_DATA/HR/{year}/' + f'*{date}.csv'):
                ...
                yh_df = pd.read_csv(f'{root_dir}ONLINE_PARSED_DATA/YH_ROSTERS/{year}/YH_stats_{date}.csv')
                ...
                if year < 2014:
                    hr_nst_yh_df.columns = ['Player_HR', 'G_HR', 'A_HR', 'PTS_HR', '+/-_HR', 'PIM_HR', 'EV_HR', 'PP_HR',
                                            ...
                                            'Year', 'Week_y', 'DB', 'clean_name_yh', 'clean_name_yh_converted']
                ...
                hr_nst_yh_df['Year'].fillna(year, inplace=True)
                hr_nst_yh_df['Team_Yahoo'].fillna('Free Agency', inplace=True)
                print(
...
def player_metadata_parser(yearsToCheck, control_file):
...
def draft_analytics(year, control_file):
    if control_file['draft_analytics_status']:
        ...
        df_full = df_rosters.merge(df_current_draft, left_on='Player_Yahoo', right_on='name')[
...
def yahoo_win_losser(df, cf, year):
    ...
    for uid in df['UID'].unique():
        ...
        for col in ['Goals_NST', 'Total Assists_NST',
            ...
            df.loc[(df['UID'] == uid) & (df['STATUS'] == 'ACTIVE'), 'Closeness_Score'] += closeness(col_delta, col)
...
def trending_analytics(all_df, cf):
    ...
    for team in df['Team_Yahoo'].unique():
        ...
        for week in team_df['Week']:
            ...
            # print(f"Week {week} week score: {team_week_df['WEEK_POWER_SCORE'].values[0]} avg score: {(historic_component)/week}")
...
def matchup_consolidator(year, control_file):
    if control_file['matchup_consolidator_status']:
        ...
        season['Team_Yahoo'].fillna('Free Agency', inplace=True)
...
def google_sheets_trunc_and_load(control_file):
    if control_file['cloud_upload_status']:
        ...
        for table in table_dict.keys():
            ...
            table_df = pd.read_csv(root_dir+table_dict[table])
